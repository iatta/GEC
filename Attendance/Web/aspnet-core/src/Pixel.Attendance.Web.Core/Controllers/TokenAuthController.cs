...
using Pixel.Attendance.Authentication.TwoFactor.Google;
...
using Pixel.Attendance.Web.Authentication.JwtBearer;
using Pixel.Attendance.Web.Authentication.TwoFactor;
using Pixel.Attendance.Web.Models.TokenAuth;
using Pixel.Attendance.Authorization.Impersonation;
using Pixel.Attendance.Authorization.Roles;
using Pixel.Attendance.Configuration;
using Pixel.Attendance.Debugging;
using Pixel.Attendance.Identity;
using Pixel.Attendance.Net.Sms;
using Pixel.Attendance.Notifications;
using Pixel.Attendance.Security.Recaptcha;
using Pixel.Attendance.Web.Authentication.External;
using Pixel.Attendance.Web.Common;
...
namespace Pixel.Attendance.Web.Controllers
{
    ...
    public class TokenAuthController : AttendanceControllerBase
    {
        ...
        IUserDevicesAppService _userDevicesAppService;
        ...
        [AbpAllowAnonymous]
        [HttpPost]
        public async Task<MobileResultModel> Login([FromBody] MobileLoginModel model)
        {
            try
            {
                var user = _userManager.Users.Include(u => u.JobTitle).FirstOrDefault(x => x.CivilId == model.CivilId);
               
                if (user == null) return new MobileResultModel { Status = 1, Message = (model.LanguageId == 1) ? "المستخدم غير موجود" : "No User Found " };
                if (user.IsActive == false) return new MobileResultModel { Status = 2, Message = (model.LanguageId == 1) ? "المستخدم غير نشط" : "User not active" };
                if (user.IsFaceRegistered == false) return new MobileResultModel { Status = 3, Message = (model.LanguageId == 1) ? "بصمة المستخدم غير موجوده" : "user not enrolled for facerecognition" };

                var loginResult = await GetLoginResultAsync(
                  user.EmailAddress,
                  DecryptString(user.MobilePassword),
                  GetTenancyNameOrNull()
              );

                if (!await _userDevicesAppService.IsExist(model.CivilId, model.DeviceSN)) await _userDevicesAppService.CreateOrEdit(new CreateOrEditUserDeviceDto { CivilID = model.CivilId, DeviceSN = model.DeviceSN });
                return new MobileResultModel()
                {
                    Status = 4,
                    ...
                    User = new MobileUserModel
                    {
                        ...
                        UserName = user.Name,
                        UserPic = user.Image,
                        Position = (model.LanguageId == 1) ? user.JobTitle.NameAr : user.JobTitle.NameEn,
                        CivilId = user.CivilId
                    }
                    ,
                    AccessToken = CreateAccessToken(await CreateJwtClaims(loginResult.Identity, loginResult.User))
                    //AccessToken = CreateAccessToken(new Claim[] { new Claim("civilid", model.CivilId),new Claim(AppConsts.TokenType,"0") }),
                };
            }
            catch (Exception)
            {

                return new MobileResultModel { Status = 1, Message = (model.LanguageId == 1) ? "المستخدم غير موجود" : "No User Found " };
            }
        }
