...
@Component({
    selector: 'managerUnitsModal',
    templateUrl: './manager-units-modal.component.html'
})
export class ManagerUnitsModalComponent extends AppComponentBase {

    @Input() cascadeSelectEnabled = true;
    @ViewChild('managerUnitsModal', { static: true }) modal: ModalDirective;
    ...
    private getTreeDataFromServer(): void {
        let self = this;
        this._organizationUnitService.getCurrentManagerUnits().subscribe((result: ListResultDtoOfOrganizationUnitDto) => {
            debugger
            result.items[0].parentId = null;
